...
//            header.put("Range", range);
    ...
    private UploadResult uploadBytes(String fileName, String path, byte[] bytes, String contentType, String bucketName) throws Exception {
        ...
        try {
            ...
            uploadArgs.object(path + fileName);
    ...
    public UploadResult uploadMultipartFile(MultipartFile file, String path, @Nullable String bucketName) throws Exception {
        ...
        if (Validate.validateType(file, minioProperties)) {
            ...
            return uploadBytes(file.getOriginalFilename(), path, fileData, file.getContentType(), bucketName);
        }
        throw new IllegalArgumentException("Content-type not allow");
    }

    public UploadResult uploadLargeFile(MultipartFile file, String path, @Nullable String bucketName) throws Exception {
        try {
            ...
            result.setPath(path + file.getOriginalFilename());
            result.setVersionId(response.versionId());
            return result;
